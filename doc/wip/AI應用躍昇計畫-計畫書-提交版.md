...
- **申請公司名稱：** ITracXing Co., Ltd. (艾創行股份有限公司)  
- **通訊地址：** 新北市板橋區（填寫完整地址）  
...
**公司簡介（依模板）**  
- **基本資料**  
  - 公司名稱：ITracXing Co., Ltd.  
  - 設立日期：2020.03.15  
  - 產業領域別：資訊服務業（J類；電腦系統設計）  
  - 前三大股東/持股：Jeff Shuo/60%，技術團隊/25%，策略投資人/15%  
  - 研發人員總數/全公司人數：12/15  
  - 2024實收資本額（千元）：10,000  
...
- **ITracXing**：LangChain多智能體、DQN/SAC、Kalman濾波、LLM整合、Next.js 全端、邊緣AI與韌體。  
...
- **驗證項目**：  
  - Phase I（3個月）：異常偵測準確率、NL查詢延遲、降噪SNR。  
  - Phase II（5個月）：警示誤報率、電池壽命、邊緣推論延遲。  
  - Phase III（4個月）：多智能體吞吐、聯邦學習隱私、穩定性。  
- **導入方式**：API整合（Arviem）、雲端部署、邊緣韌體OTA、分階訓練上線。

### （2）計畫工作項目實施方式（含百分比/單位）
```
Phase I: AI原型 (3個月, 27%)
- A1 自然語言查詢 (8%) – ITracXing
- A2 電池異常偵測 (10%) – ITracXing
- A3 Kalman降噪 (5%) – ITracXing
- A4 風險可視化 (4%) – ITracXing

Phase II: RL整合 (5個月, 47%)
- B1 DQN/SAC警示優化 (15%) – ITracXing + ITRI(驗證)
- B2 自適應Kalman調校 (8%) – ITracXing
- B3 邊緣RL採樣韌體 (12%) – ITracXing
- B4 邊緣推論優化 (7%) – ITracXing
- B5 聯合驗證與測試 (5%) – ITRI + 學研中心

Phase III: 多智能體部署 (4個月, 26%)
- C1 MCP多智能體協作 (8%) – ITracXing
- C2 聯邦學習POC (7%) – ITracXing + 學研中心
- C3 生產部署 (6%) – ITracXing
- C4 API與文件 (3%) – ITracXing
- C5 驗收與交付 (2%) – ITracXing + ITRI
```
...
### 資訊安全運作機制說明
- 安全開發流程（SSDLC），密鑰/機密以 HSM/雲KMS管理。  
- 存取控管（RBAC）、稽核日誌、資料最小權限與加密（傳輸/靜態）。  
- 邊緣裝置韌體簽章與OTA驗證；聯邦學習保護原始資料不出域。

## 四、計畫執行時程及查核點

### 預定進度表（年度=會計年度；每6個月至少1項查核點）
- **2025/11–2026/01（Q1）**：完成 A1–A3 原型與指標測試（查核點 A.1）  
- **2026/02–2026/04（Q2）**：完成 B1 訓練與A/B測試、B3 初版韌體（查核點 B.1）  
- **2026/05–2026/07（Q3）**：完成 B4 邊緣推論優化、B5 聯合驗證（查核點 B.2）  
- **2026/08–2026/10（Q4）**：完成 C1–C5 生產化、最終驗收（查核點 C.1；結案驗收）  

### 預定查核點說明（舉例）
- **A.1（2026/01）**：NL查詢<2秒；異常偵測準確率≥85%；SNR提高≥20%。  
- **B.1（2026/04）**：警示誤報率≤20%；F1-score≥0.85；邊緣推論延遲<10ms。  
- **B.2（2026/07）**：電池壽命延長≥40%；自適應Kalman RMSE顯著下降。  
- **C.1（2026/10）**：多智能體協作吞吐量達標；聯邦學習隱私測試通過；完成最終驗收。
...
## 一、計畫主持人資歷
- 姓名：Jeff Shuo｜職稱：執行長｜性別：□男 □女 □其他（擇一）｜產業領域：資訊服務  
